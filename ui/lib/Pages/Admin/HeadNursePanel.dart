  ...
  Future<void> _createSupervision() async {
    try {
      ...
      if (headNurseEmail.isEmpty) {
        throw Exception('Geef een hoofdverpleger in.');
      }

      final headNurse = await UserService.getUserByEmail(headNurseEmail);
      if (headNurse.function != FunctionType.Headnurse) {
        throw Exception('De eerste gebruiker moet een geldige hoofdverpleger zijn.');
      }
      ...
      if (!hasValidEntries) {
        throw Exception('Geef minstens een verpleegkundige of patiënt in.');
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error bij opdracht: $e')),
      );
    }
  }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildSectionTitle('Hoofdverpleger'),
              _buildInputField(headNurseController, 'Hoofdverpleger email (verplicht)'),
